...
public partial class MainWindow : Window {
    ...
    private void BanUserButton_Click(object sender, RoutedEventArgs e) {
        var dialog = new BanUserDialog();
        if (dialog.ShowDialog() == true) {
            var username = dialog.OffendersUsername;
            var unbanDate = dialog.DateOfUnban;
            if (Server.AuthService.TryBanUser(username, unbanDate)) {
                string? until = dialog.DateOfUnban == null ? "FOREVER" : dialog.DateOfUnban.ToString();
                MessageBox.Show($"'{dialog.OffendersUsername}' will be banned until {until}");
            } else {
